...
if ("_scr" != pic1[-8:-4]): 
    ...
    # Normalize img1 for warping
    img1_float = img1.astype(float)/255.0
    scr = skimage.transform.warp(img1_float, PT, output_shape=(img1.shape[0], img1.shape[1]))
    scr = (scr*255).astype(np.uint8)

    fname = pic1[:-4]+"_scr.jpg"
    skimage.io.imsave(fname, scr)
    kp_1, des_1, scr_img = getSIFTKPfromIMG(scr, 1000)

    dims = np.array([xs,ys,[row,col,upp,0]])
    fname = pic1[:-4]+"_info.txt"
    np.savetxt(fname,dims)

    # scr_img returned from SIFT function is RGB
    img1 = scr_img
else:
